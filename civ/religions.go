...
func (m *Civ) PlaceNOrganizedReligions(n int) []*Religion {
	var religions []*Religion
	cities := make([]*City, len(m.Cities.Objects))
	copy(cities, m.Cities.Objects)
...
func (m *Civ) placeReligionAt(r int, founded int64, group string, culture *Culture, parent *Religion) *Religion {
	...
	if group == genreligion.GroupOrganized {
		nameGen, expansion := m.getOrganizedReligionName(rlgGen, culture, relg.Deity, relg.Classification, r)
		...
		nameGen, expansion := m.getFolkReligionName(rlgGen, culture, relg.Deity, relg.Classification, r)
		relg.Name = nameGen.Text
		relg.NameGen = nameGen
		relg.Expansion = expansion
	...
	if parent != nil {
		m.History.AddEvent("Founding (Religion)", fmt.Sprintf("%s split from %s", relg.Name, parent.Name), relg.Ref())
	} else {
		m.History.AddEvent("Founding (Religion)", fmt.Sprintf("%s was founded", relg.Name), relg.Ref())
	}
	m.Religions.PlaceObjectAt(relg, r)
	m.Religions.Regions[r] = -1 // TODO: Fixt this. The expansion doesn't work if the region is already taken.
	return relg
}

// GetID returns the ID of the religion.
func (r Religion) GetID() int {
	return r.ID
}

// Ref returns a reference to the religion.
func (r *Religion) Ref() ObjectReference {
...
func (m *Civ) genOrganizedReligion(c *City) *Religion {
	var parent *Religion
	if rID := m.Religions.Regions[c.ID]; rID >= 0 {
