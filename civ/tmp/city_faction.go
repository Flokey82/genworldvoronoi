...
func (f *Faction) TickCity(c *City, m *Civ) {
	...
	if CityFeatureQuarry.Requires(c, m) {
		c.Features.Add(CityFeatureQuarry)
		// Add a history entry.
		m.History.AddEventf("CONSTRUCTION", c.Ref(), "The %s faction has constructed a quarry.", f.Name)
	} else if CityFeatureMine.Requires(c, m) {
		c.Features.Add(CityFeatureMine)
		// Add a history entry.
		m.History.AddEventf("CONSTRUCTION", c.Ref(), "The %s faction has constructed a mine.", f.Name)
	} else if CityFeatureLumberyard.Requires(c, m) {
		c.Features.Add(CityFeatureLumberyard)
		// Add a history entry.
		m.History.AddEventf("CONSTRUCTION", c.Ref(), "The %s faction has constructed a lumberyard.", f.Name)
	} else if CityFeatureGemMine.Requires(c, m) {
		c.Features.Add(CityFeatureGemMine)
		// Add a history entry.
		m.History.AddEventf("CONSTRUCTION", c.Ref(), "The %s faction has constructed a gem mine.", f.Name)
	} else if CityFeatureAquaeduct.Requires(c, m) {
		c.Features.Add(CityFeatureAquaeduct)
		// Add a history entry.
		m.History.AddEventf("CONSTRUCTION", c.Ref(), "The %s faction has constructed an aquaeduct.", f.Name)
	}

	// Tick all the features.
	// TODO: Move this to a separate function.
	for _, feat := range c.Features.feats {
		feat.Effect(c, m)
	}
}
