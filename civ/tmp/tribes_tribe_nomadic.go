...
func (s *simState) handleNomadicTribe(t *Tribe) {
	...
	for i, nb := range neigbors {
		...
		if s.m.Elevation.Values[nb] > 0 && s.tribeAtRegion[nb] == nil {
			maxPopPerRegion[i] = s.m.calcMaxPopPerRegion(t, nb)
		}
	}
	...
	for _, idx := range neighborIndices {
		...
		if nbMaxPop >= tCurrent.Population {
			...
			// compared to the current region, etc.
		...
		// Change the satisfaction of the tribes.
	...
	if tCurrent != nil {
		...
		if maxPop := s.m.calcMaxPopPerRegion(t, tCurrent.RegionID); maxPop <= 0 {
