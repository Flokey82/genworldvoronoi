...
type Skill struct {
	...
	RegProx        geo.RegionProximity // The region proximity required to develop the skill.
...
func (s *Skill) CanDevelopIn(curRegProp *geo.RegionProp, resPres *ResourcePresence) bool {
...
func (s *Skill) GetBonusForBiome(curRegProp *geo.RegionProp, resPres *ResourcePresence) float64 {
...
var (
	...
	SSkillBartering = &Skill{
		Name:       "Bartering",
		MinScore:   0.5,
		Chance:     0.4,
		Requires:   []*Skill{SSkillGathering},
		Profession: ProfessionMerchant,
	}
	SSkillTrading = &Skill{
		Name:     "Trading",
		MinScore: 0.5,
		Chance:   0.4,
		Requires: []*Skill{SSkillBartering},
		// Profession: ProfessionMoneylender,
	}
...
func (m *Civ) genCultureSkills() {
	...
	cultureCopy := make([]*Culture, len(m.Cultures.Objects))
	copy(cultureCopy, m.Cultures.Objects)
	...
	for _, res := range m.Resources {
		sort.Slice(cultureCopy, func(i, j int) bool {
			return float64(cultureCopy[i].Stats.Resouces[res])/float64(len(cultureCopy[i].Regions)) > float64(cultureCopy[j].Stats.Resouces[res])/float64(len(cultureCopy[j].Regions))
		})
