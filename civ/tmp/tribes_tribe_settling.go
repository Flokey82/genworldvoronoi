...
func (s *simState) handleEstablishingSettlement(t *Tribe) {
	...
	ctf := GetRegionCultureTypeFunc(s.m.Geo) // <- VERY COSTLY!!!
	...
	if t.Culture == nil {
		if existingCulture == nil || existingCulture.Type != regionType {
			// No existing regional culture or the existing culture isn't the right type
	...
	if t.Settlement != nil {
		...
		t.Settlement = s.m.placeCityAt(t.RegionID, -1, s.m.getRegCityType(t.RegionID), t.Population, 1.0, t.Leadership)
		s.m.calculateCitiesStats([]*City{t.Settlement})

		// Log the stats of the new settlement.
		log.Printf("New Settlement %s in region %d agriculture %.2f, attractiveness %.2f", t.Settlement.Name, t.RegionID, t.Settlement.Agricultural, t.Settlement.Attractiveness)

		// Get secondary type of the settlement, which is the types of neighboring regions.
		seenSecondaryTypes := make(map[CityType]bool)
	...
	// Check if the tribe has a city state or empire, which should not be the case.
	if t.CityState != nil || t.Empire != nil {
		panic("Tribe is a city state or empire")
	}

	// Set the tribe as a settled tribe and evolve the government form.
	t.doneSettling = true
	...
	if t.gotVision {
		...
		t.Religion = s.m.placeReligionAt(t.RegionID, 0, group, t.Culture, t.Religion)
...
func (s *simState) getSettleScoreFunc(t *Tribe, spiritualCalling bool) func(r int) (float64, bool) {
	...
	settleRegionScore := func(r int) (float64, bool) {
		...
		if dist > radius {
			return 0, false // The region is too far away.
		}
		...
		minDist := math.Inf(1)
		for _, c := range s.m.Cities.Objects {
			if dist := s.m.Geo.GetDistance(c.ID, r) * unitDistToKm; dist < minDist {
				minDist = dist
			}
		}
		minDist = min(radiusMaxCityProx, minDist) // Cap the distance to 300 km (for now)
		log.Printf("Tribe %d has scouted region %d (%.2f km away), and the distance to the closest city is %.2f km.", t.ID, r, dist, minDist)
		...
		maxPopPerReg := float64(min(s.m.calcMaxPopPerRegion(t, r), 2*t.Population))
		regMul := s.m.getRegMultiplier(t, r)
...
func (s *simState) findNewRegionToSettle(t *Tribe, avoidRegs []int) bool {
	...
	logInfo := false
	if logInfo {
		s.m.printRegionInfo(bestRegion)
