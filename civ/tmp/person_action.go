...
func pickVictim(m *Civ, p *Person) *relation {
	...
	if rand.Intn(100) < 50 {
		...
		if len(filteredOptions) > 0 {
			...
			// TODO: Maybe add some criteria, so we don't pick a person we actually like
			// except if we are actually cruel and impulsive.
	...
	// Log the filtered nearby people with the values.
	// for i, person := range filteredNearby {
	// 	log.Printf("%d: Person: %s, Opinion: %.2f, Comparison: %.2f, Sum: %.2f", i, person.Name(), p.Opinions.GetOpinion(person), p.compare(person), p.Opinions.GetOpinion(person)+p.compare(person))
	// }
...
var (
	actionChangeName = &PersonalAction{
		probability: func(p *Person) float64 {
			// We might change our name, either because we had a falling out with our family,
			// because we want to start a new life, or because we want to hide our identity.
			// For now we only change the name if we dislike both of our parents.
			prob := 0.001
			if p.Mother != nil && p.Father != nil {
				// Check if we have the same last name as our parents.
		...
		consequences: func(m *Civ, p *Person) {
			...
			// Add event.
			ev := m.AddEvent("Name Change", fmt.Sprintf("%s changed their name to %s", origName, p.Name()), p.Ref())
	...
	actionHeroAdventure = &PersonalAction{
		probability: func(p *Person) float64 {
			...
			// If we have a spouse or children, we are less likely to go on an adventure.
			if p.Spouse != nil {
				prob -= 0.2
			}
			if len(p.Children) > 0 {
			...
			// TODO: We might be more likely if we are down on our luck.
			return max(prob, 0)
		},
		requires: func(p *Person) bool {
			return p.Age > 12
		...
		consequences: func(m *Civ, p *Person) {
			// Pick an adventure.
			// This should depend on the region, the person's traits, etc.
			// For now we just call it "adventure".
			historyMsg := fmt.Sprintf("%s went on an adventure", p.Name())

			// Add event.
			ev := m.AddEvent("Adventure", historyMsg, p.Ref())
			...
			if rand.Intn(100) < 50 {
				...
				if person != nil {
					...
					if p.Traits.HasTrait(geneticshuman.TraitKind) || p.Traits.HasTrait(geneticshuman.TraitHonest) || p.Opinions.GetOpinion(person) > 0.5 {
						...
						// Maybe add a little buff or positive memory.
					}
				}
			}
		},
	...
	actionChildPlay = &PersonalAction{
		...
		consequences: func(m *Civ, p *Person) {
			// Pick a random child.
			peopleNearby := m.getPeopleAt(p.Region)
			children := make([]*Person, 0, len(peopleNearby)/4)
			...
			// Sort the children by opinion (highest first).
	...
	actionVillainChildBully = &PersonalAction{
		...
		consequences: func(m *Civ, p *Person) {
			...
			// Sort the children by opinion (lowest first).
			sort.Slice(children, func(i, j int) bool {
				return p.Opinions.GetOpinion(children[i]) < p.Opinions.GetOpinion(children[j])
			})
