...
func (m *Civ) getFolkReligionName(rlgGen *genreligion.Generator, c *Culture, deity *genreligion.Deity, rel *genreligion.Classification, r int) (*genstory.Generated, string) {
...
func (m *Civ) getOrganizedReligionName(rlgGen *genreligion.Generator, c *Culture, deity *genreligion.Deity, rel *genreligion.Classification, r int) (*genstory.Generated, string) {
	// Returns the name of the city, -state, or culture at the given region.
	place := func() string {
		// Get the name of the city at the region.
		if city := m.GetCity(r); city != nil {
			return city.Name
		}

		// If unsuccessful, Check if we have a state at the region
		// and use the capital name.
		if state := m.CityStates.Get(r); state != nil {
			return state.Capital.Name
		}
	...
	tokens = append(tokens, genstory.TokenReplacement{
		...
		Replacement: c.Language.MakeName(), // TODO: Use the religion generator to generat religous terms.
	})
